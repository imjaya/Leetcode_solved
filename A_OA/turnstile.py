...
def turnstile(times, dirs):
    ...
    for currTime in range(0, endTime+1, 1):
        while currTime == times[currIdx]:
            if dirs[currIdx] == 0:
                inque.append(currIdx)
            else: 
                outque.append(currIdx)
            currIdx += 1
            if currIdx == len(times): break
        
        if len(inque) + len(outque) == 0:
            prevDir = 1
        
        if prevDir == 0 or len(outque) == 0:
            if len(inque) > 0:
                index = inque.pop(0)
                res[index] = currTime
                prevDir = 0
